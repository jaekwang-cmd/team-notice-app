...
const CHANGELOG = {
  ...
  '0.36.1': [
    '고객 리마인더: 연락처 입력 시 자동으로 하이픈이 붙습니다',
    '고객 리마인더: 검색창을 추가했습니다 (고객명/연락처/차종/메모)',
    '고객 리마인더: 목록이 "이번 달/다음 달/그 이후" 대신 실제 연·월별로 나뉘어 보입니다',
    '고객 리마인더: 카드에 마우스를 올려도 삭제 버튼이 안 보이던 문제를 고쳤습니다',
  ],
  '0.36.2': [
  ...
  '0.39.6': [
    '메뉴·본문 글씨를 최대 200%까지 확대하고 풍경 버튼과 메모의 작은 글씨도 함께 키울 수 있습니다',
    '손글씨와 다이어리 글씨 등 서체 8종을 추가했습니다',
  ],
  '0.39.7': [
    '정산서 미리보기를 Ctrl+마우스 휠로 확대·축소할 수 있습니다',
    '오늘 날짜 줄바꿈을 개선하고 풍경 조작 바를 자동 전환으로 정리했습니다',
    '오늘의 페이지 오른쪽에서 AI 비서와 바로 대화할 수 있습니다',
  ],
  '0.39.8': [
    '연속 일정 저장 시 이전 요청이 새 입력창을 닫거나 저장 버튼을 계속 잠그는 문제를 수정했습니다',
    '일정 삭제 확인창을 화면 실행을 중단하지 않는 방식으로 변경했습니다',
  ],
  '0.39.9': [
    '오늘의 페이지에서 저장·조회 중에도 다음 일정 초안을 입력할 수 있도록 수정했습니다',
    '달력 갱신 요청을 순차 처리하고 메인 화면의 동기식 확인창을 비동기 방식으로 교체했습니다',
  ],
};
...
찾거나 열어달라고 하면 search_files 도구를 쓴다. "폴스타 견적기"처럼 동사 없이 파일/서류 이름만
툭 던지듯 말해도 파일을 찾아달라는 뜻으로 보고 바로 search_files를 호출한다 — 애매하다고 대화로
되묻거나, 도구를 부르지도 않고 "찾을 수 없었습니다/없는 파일입니다"라고 단정하지 마라. 실제로
찾아본 적이 없다면 결과를 안다고 말할 수 없다는 뜻이다. 찾은 목록은 사용자 화면에 클릭해서 바로
열 수 있는 형태로 표시되므로, 답변에서는 몇 건 찾았는지와 눌러서 열면 된다는 정도만 짧게 안내한다 —
